...
use super::{
	...
	util::{
		edit_reply, format_play_eval_stderr, generic_help,
		hoise_crate_attributes, parse_flags, stub_message, GenericHelp,
	},
};
...
pub async fn microbench(
	...
	let pub_fn_indices =
		user_code.match_indices("pub fn ").collect::<Vec<_>>();
	match pub_fn_indices.len() {
		0 => {
			ctx.say(
				"No public functions (`pub fn`) found for benchmarking \
				 :thinking:",
			)
			.await?;
			return Ok(());
		...
		1 => {
			ctx.say(
				"Please include multiple functions. Times are not \
	...
	for (index, _) in pub_fn_indices {
		...
		let function_name_end =
			match user_code[function_name_start..].find('(') {
				Some(x) => x + function_name_start,
				None => continue,
			};
		let function_name =
			user_code[function_name_start..function_name_end].trim();
	...
	let code =
		hoise_crate_attributes(user_code, after_crate_attrs, &after_code);

	let (flags, mut flag_parse_errors) =
	...
	if black_box_hint {
		flag_parse_errors += "Hint: use the black_box function to prevent \
...
pub fn microbench_help() -> String {
	generic_help(GenericHelp {
		command: "microbench",
		desc: "\
Benchmarks small snippets of code by running them repeatedly. Public \
		       functions are run in blocks of 1000 repetitions in a cycle \
		       until 5 seconds have passed. Measurements are averaged and \
		       standard deviation is calculated for each

Use the `std::hint::black_box` function, which is already imported, to wrap \
		       results of computations that shouldn't be optimized out. \
		       Also wrap computation inputs in `black_box(...)` that should \
		       be opaque to the optimizer: `number * 2` produces optimized \
		       integer doubling assembly while `number * black_box(2)` \
		       produces a generic integer multiplication instruction",
