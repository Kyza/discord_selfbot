...
pub fn parse_flags(
	mut args: poise::KeyValueArgs,
) -> (api::CommandFlags, String) {
...
pub fn hoise_crate_attributes(
	code: &str,
	after_crate_attrs: &str,
	after_code: &str,
) -> String {
...
pub fn maybe_wrapped(
	...
	use syn::{parse_str, Attribute, Block, Item, ItemFn, Stmt};
...
pub async fn edit_reply(
	...
	let result = crate::helpers::merge_output_and_errors(
		&result.stdout,
		&result.stderr,
	);
	...
	if result.trim().is_empty() {
		message
			.edit(
				ctx,
				CreateReply::default()
					.content(format!("{flag_parse_errors}``` ```")),
			)
			.await?;
		return Ok(());
	}

	let timeout = result.contains("Killed")
		&& result.contains("timeout")
		&& result.contains("--signal=KILL");
	...
	let text = crate::helpers::trim_text(
		...
		async {
			format!(
				...
				api::url_from_gist(
					flags,
					&api::post_gist(ctx, code).await.unwrap_or_default()
				),
			)
		},
	)
	...
	message
		.edit(ctx, {
			let mut b = CreateReply::default().content(text);
			if timeout {
				b = b.components(vec![serenity::CreateActionRow::Buttons(
					vec![serenity::CreateButton::new(&custom_id)
						.label("Retry")
	...
	if let Some(retry_pressed) = message
		.message()
		.await?
		.await_component_interaction(ctx)
		.filter(move |mci: &ComponentInteraction| {
			mci.data.custom_id == custom_id
		})
...
pub fn format_play_eval_stderr(
	stderr: &str,
	show_compiler_warnings: bool,
) -> String {
	...
	if stderr.contains("Finished ") {
		...
		let program_stderr = extract_relevant_lines(
			stderr,
			&["Finished ", "Running `target"],
			&[],
		);
...
pub fn stub_message(ctx: Context<'_>) -> String {
	...
	if let Context::Prefix(ctx) = ctx {
		if let Some(edit_tracker) =
			&ctx.framework.options().prefix_options.edit_tracker
		{
