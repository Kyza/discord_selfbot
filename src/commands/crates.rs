...
use crate::{serenity, types::Context};
...
async fn get_crate(http: &reqwest::Client, query: &str) -> Result<Crate> {
	...
	let crate_list = http
		...
		.await
		.map_err(|e| {
			anyhow!("Cannot parse crates.io JSON response (`{}`)", e)
		})?;
...
pub async fn doc(
	...
	let mut doc_url = if let Some(rustc_crate) =
		rustc_crate_link(first_path_element)
	{
		...
		get_documentation(
			&get_crate(&ctx.data().http, first_path_element).await?,
		)
	};
